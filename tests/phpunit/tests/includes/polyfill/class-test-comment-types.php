...
class Test_Comment_Types extends \WP_UnitTestCase {
	 ...
	 * undefined by both the polyfill and the shim, so neither branch of the guard exercises the
	 * stand-down. The predicate is filterable for exactly this reason: the test forces it true for
	 * its duration and checks each hook returns its input untouched. If it ever failed, a hook
	 * would be pre-empting core's own query with a copy of it.
	 */
	public function test_hooks_stand_down_when_core_reads_the_set() {
		...
		$this->assertArrayNotHasKey( 'type__not_in', \Activitypub\Comment::rest_comment_query( array( 'type' => '' ) ), 'rest_comment_query() adds no type__not_in.' );
		...
		\remove_filter( 'activitypub_core_reads_excluded_comment_types', '__return_true' );
	}
