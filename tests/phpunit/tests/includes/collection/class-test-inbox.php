...
class Test_Inbox extends \WP_UnitTestCase {
	...
	/**
	 * A second actor reusing a stored activity ID must not gain its recipients.
	 *
	 * @covers ::add
	 */
	public function test_add_rejects_activity_id_claimed_by_another_actor() {
		$activity_id = 'https://remote.example.com/activities/claimed';

		$first = new Activity();
		$first->set_id( $activity_id );
		$first->set_type( 'Create' );
		$first->set_actor( 'https://remote.example.com/users/alice' );

		$note = new Base_Object();
		$note->set_id( 'https://remote.example.com/objects/note' );
		$note->set_type( 'Note' );
		$note->set_content( 'Original content' );
		$first->set_object( $note );

		$inbox_id = Inbox::add( $first, 1 );
		$this->assertIsInt( $inbox_id );

		$collision = new Activity();
		$collision->set_id( $activity_id );
		$collision->set_type( 'Create' );
		$collision->set_actor( 'https://attacker.example.com/users/mallory' );

		$decoy = new Base_Object();
		$decoy->set_id( 'https://attacker.example.com/objects/decoy' );
		$decoy->set_type( 'Note' );
		$collision->set_object( $decoy );

		$result = Inbox::add( $collision, 2 );

		$this->assertWPError( $result );
		$this->assertSame( 'activitypub_inbox_actor_mismatch', $result->get_error_code() );

		// The second user must not have been attached to the first actor's activity.
		$recipients = Inbox::get_recipients( $inbox_id );
		$this->assertSame( array( 1 ), $recipients );

		// The stored activity must still be the one its actor sent.
		$stored = \json_decode( Inbox::get( $inbox_id )->post_content, true );
		$this->assertSame( 'https://remote.example.com/users/alice', $stored['actor'] );
		$this->assertSame( 'Original content', $stored['object']['content'] );
	}

	/**
	 * The same actor redelivering an activity must still collect the new recipient.
	 *
	 * @covers ::add
	 */
	public function test_add_merges_recipients_for_same_actor() {
		$activity_id = 'https://remote.example.com/activities/redelivered';

		$activity = new Activity();
		$activity->set_id( $activity_id );
		$activity->set_type( 'Create' );
		$activity->set_actor( 'https://remote.example.com/users/alice' );

		$note = new Base_Object();
		$note->set_id( 'https://remote.example.com/objects/redelivered' );
		$note->set_type( 'Note' );
		$activity->set_object( $note );

		$first  = Inbox::add( $activity, 1 );
		$second = Inbox::add( $activity, 2 );

		$this->assertSame( $first, $second );

		$recipients = Inbox::get_recipients( $first );
		$this->assertCount( 2, $recipients );
		$this->assertContains( 1, $recipients );
		$this->assertContains( 2, $recipients );
	}
	...
	/**
	 * Rows sharing an ID but not an actor are not the race deduplicate resolves.
	 *
	 * @covers ::deduplicate
	 */
	public function test_deduplicate_keeps_rows_from_a_different_actor() {
		$guid = 'https://remote.example.com/activities/contested';
		...
		\add_post_meta( $primary_id, '_activitypub_activity_remote_actor', 'https://remote.example.com/users/alice' );
		\add_post_meta( $primary_id, '_activitypub_user_id', 1 );

		$impostor_id = \wp_insert_post(
			array(
				'post_type'   => Inbox::POST_TYPE,
				'post_status' => 'publish',
				'guid'        => $guid,
		...
		\add_post_meta( $impostor_id, '_activitypub_activity_remote_actor', 'https://attacker.example.com/users/mallory' );
		\add_post_meta( $impostor_id, '_activitypub_user_id', 2 );

		$result = Inbox::deduplicate( $guid );

		$this->assertInstanceOf( 'WP_Post', $result );
		$this->assertEquals( $primary_id, $result->ID );

		// The other actor's recipient must not be merged in, and its row must survive.
		$this->assertSame( array( 1 ), Inbox::get_recipients( $primary_id ) );
		$this->assertInstanceOf( 'WP_Post', \get_post( $impostor_id ) );
	}
}
