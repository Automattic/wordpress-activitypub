...
class Test_Posts extends \WP_UnitTestCase {
	...
	/**
	 * Test that adding with empty recipients returns error.
	 *
	 * @covers ::add
	 */
	public function test_add_with_empty_recipients_returns_error() {
		$activity = array(
			'object' => array(
				'id'           => 'https://example.com/objects/no-recipients',
				'type'         => 'Note',
				'name'         => 'No Recipients',
				'content'      => '<p>Test no recipients</p>',
				'attributedTo' => 'https://example.com/users/testuser',
			),
		);

		$result = Posts::add( $activity, array() );

		$this->assertInstanceOf( '\WP_Error', $result );
		$this->assertEquals( 'activitypub_posts_no_recipients', $result->get_error_code() );
	}

	/**
	 * Test that updating with empty recipients returns error.
	 *
	 * @covers ::update
	 */
	public function test_update_with_empty_recipients_returns_error() {
		$activity = array(
			'object' => array(
				'id'           => 'https://example.com/objects/update-no-recipients',
				'type'         => 'Note',
				'name'         => 'Original',
				'content'      => '<p>Original</p>',
				'attributedTo' => 'https://example.com/users/testuser',
		...
		$result = Posts::update( $update_activity, array() );
