...
class Test_Sanitize extends \WP_UnitTestCase {
	...
	public function test_remove_hashtags( $content, $hashtags, $expected ) {
		$result = Sanitize::remove_hashtags( $content, $hashtags );
	...
	public function test_remove_hashtags_with_invalid_hashtags() {
		...
		$this->assertEquals( $content, Sanitize::remove_hashtags( $content, 'not-an-array' ) );
		$this->assertEquals( $content, Sanitize::remove_hashtags( $content, null ) );
		$this->assertEquals( $content, Sanitize::remove_hashtags( $content, 123 ) );
	}
	...
	public function test_remove_hashtags_preserves_structure() {
		...
		$result  = Sanitize::remove_hashtags( $content, array( 'test', 'php' ) );
