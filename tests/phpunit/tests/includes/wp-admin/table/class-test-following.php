...
class Test_Following extends \WP_UnitTestCase {
	...
	/**
	 * Test that attempting to follow an already-followed account shows correct message.
	 *
	 * @covers ::process_action
	 */
	public function test_follow_already_followed_account() {
		// Create a mock actor.
		$actor_data = array(
			'id'                => 'https://example.com/users/followed',
			'type'              => 'Person',
			'name'              => 'Already Followed User',
			'preferredUsername' => 'followed',
			'inbox'             => 'https://example.com/users/followed/inbox',
		);
		...
		// Set up the request to try to follow again.
		$_REQUEST['action']              = 'follow';
		$_REQUEST['activitypub-profile'] = 'https://example.com/users/followed';
		$_REQUEST['_wpnonce']            = wp_create_nonce( 'activitypub-follow-nonce' );

		// Process the action.
		$this->following_table->process_action();

		// Get settings errors.
		$errors = get_settings_errors( 'activitypub' );

		// Verify we got an info message (not error).
		$this->assertNotEmpty( $errors );
		$this->assertEquals( 'info', $errors[0]['type'] );
		$this->assertStringContainsString( 'You are already following', $errors[0]['message'] );
		$this->assertStringNotContainsString( 'pending', $errors[0]['message'] );
	...
	/**
	 * Test that attempting to follow an account with pending request shows correct message.
	 *
	 * @covers ::process_action
	 */
	public function test_follow_pending_account() {
		// Create a mock actor.
		$actor_data = array(
			'id'                => 'https://example.com/users/pending',
			'type'              => 'Person',
			'name'              => 'Pending User',
			'preferredUsername' => 'pending',
			'inbox'             => 'https://example.com/users/pending/inbox',
		);

		$actor_post_id = Remote_Actors::upsert( $actor_data );

		// Manually set as pending (not accepted yet).
		add_post_meta( $actor_post_id, Following_Collection::PENDING_META_KEY, get_current_user_id() );

		// Set up the request to try to follow.
		$_REQUEST['action']              = 'follow';
		$_REQUEST['activitypub-profile'] = 'https://example.com/users/pending';
		$_REQUEST['_wpnonce']            = wp_create_nonce( 'activitypub-follow-nonce' );
		...
		$this->assertNotEmpty( $errors );
		$this->assertEquals( 'info', $errors[0]['type'] );
		$this->assertStringContainsString( 'pending follow request', $errors[0]['message'] );
		$this->assertStringNotContainsString( 'You are already following', $errors[0]['message'] );

		// Clean up.
		delete_post_meta( $actor_post_id, Following_Collection::PENDING_META_KEY, get_current_user_id() );
	}
