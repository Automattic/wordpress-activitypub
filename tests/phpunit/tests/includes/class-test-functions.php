...
class Test_Functions extends \WP_UnitTestCase {
	...
	/**
	 * Test building a class list.
	 *
	 * @dataProvider css_class_provider
	 * @covers \Activitypub\get_css_class
	 *
	 * @param string|string[] $css_class The classes the caller adds.
	 * @param array           $expected  The expected class list.
	 */
	public function test_get_css_class( $css_class, $expected ) {
		$this->assertSame( $expected, \array_values( \Activitypub\get_css_class( $css_class ) ) );
	}
	 ...
	 * @return array[]
	 */
	public function css_class_provider() {
		return array(
			'nothing of its own'    => array( '', array() ),
			'a single class'        => array( 'u-quotation-of', array( 'u-quotation-of' ) ),
			'several, as core does' => array( 'u-in-reply-to  h-cite', array( 'u-in-reply-to', 'h-cite' ) ),
			'an array'              => array( array( 'u-in-reply-to', 'h-cite' ), array( 'u-in-reply-to', 'h-cite' ) ),
			'no duplicates'         => array( 'h-cite h-cite', array( 'h-cite' ) ),
			'sanitized'             => array( '"><script>', array( 'script' ) ),
			'a mixed array'         => array( array( 'activitypub-embed', 'u-in-reply-to h-cite' ), array( 'activitypub-embed', 'u-in-reply-to', 'h-cite' ) ),
		);
	}

	/**
	 * Test that a class list can be filtered, and that the filter knows the context.
	 *
	 * @covers \Activitypub\get_css_class
	 */
	public function test_get_css_class_is_filterable() {
		$filter = function ( $classes, $css_class, $context ) {
			$this->assertSame( array( 'u-quotation-of' ), $css_class, 'The filter sees what the caller asked for.' );
			$this->assertSame( 'quote', $context, 'The filter can tell the call sites apart.' );

			return \array_merge( $classes, array( 'p-name' ) );
		};

		\add_filter( 'activitypub_css_class', $filter, 10, 3 );
		$classes = \Activitypub\get_css_class( 'u-quotation-of', 'quote' );
		\remove_filter( 'activitypub_css_class', $filter, 10 );

		$this->assertSame( array( 'u-quotation-of', 'p-name' ), \array_values( $classes ) );
	}
}
