...
';
	...
	/**
	 * Test HTTP signature verification with invalid digest.
	 *
	 * @covers ::verify_http_signature
	 * @covers ::generate_digest
	 * @covers ::generate_signature
	 */
	public function test_verify_http_signature_with_invalid_digest() {
		// Create a user and get their keypair.
		$keys = Signature::get_keypair_for( 1 );
		...
		$modified_body = '{"type":"Create","actor":"https://example.org/author/admin","object":{"type":"Note","content":"Modified content."}}';

		$request = new \WP_REST_Request( 'POST', ACTIVITYPUB_REST_NAMESPACE . '/inbox' );
		$request->set_body( $modified_body );
		$request->set_header( 'Date', $date );
		$request->set_header( 'Digest', $digest ); // Original digest that doesn't match modified body.
		$request->set_header( 'Signature', $signature );
		$request->set_header( 'Host', 'example.org' );
