...
import type { SocialWebSelectors, SocialWebActions } from '../../store';
import SiteIcon from '../site-icon';
import './style.scss';

export default function ActorSwitcher() {
	const { defaultAvatar, adminUrl } = useSettings();
	const { setActiveActor } = useDispatch( STORE_NAME ) as SocialWebActions;

	const { currentUser, activeActorId, canManageSite } = useSelect(
		( select ) => ( {
			currentUser: select( coreStore ).getCurrentUser(),
			activeActorId: ( select( STORE_NAME ) as SocialWebSelectors ).getActiveActorId(),
