...
export default function NavigationItem( {
	...
	const isExternal = ! String( new URL( to, window.location.origin ) ).startsWith( window.location.origin );
	...
	if ( isExternal ) {
		...
		return (
			<Item as="a" href={ to } className={ clsx( 'boot-navigation-item', className ) }>
				{ content }
			</Item>
		);
	}
	...
	return (
		<RouterLinkItem to={ to } className={ clsx( 'boot-navigation-item', className ) }>
			{ content }
		</RouterLinkItem>
	);
}
