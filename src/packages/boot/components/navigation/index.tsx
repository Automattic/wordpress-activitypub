...
function Navigation() {
	...
	const parent = useMemo( () => menuItems.find( ( item ) => item.id === parentId ), [ menuItems, parentId ] );
	...
	const handleNavigate = ( { id, direction }: { id?: string; direction: 'forward' | 'backward' } ) => {
		setAnimationDirection( direction );
		setParentId( id );
	};

	const handleDropdownToggle = ( dropdownId: string ) => {
		setParentDropdownId( parentDropdownId === dropdownId ? undefined : dropdownId );
	};

	const items = useMemo( () => menuItems.filter( ( item ) => item.parent === parentId ), [ menuItems, parentId ] );
