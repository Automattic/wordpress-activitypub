 ...
 */
export function wrapIcon( icon?: IconType, shouldShowPlaceholder: boolean = true ) {
	if ( isSvg( icon ) ) {
		return <Icon icon={ icon } />;
	}

	if ( typeof icon === 'string' && icon.startsWith( 'dashicons-' ) ) {
		const iconKey = icon.replace( /^dashicons-/, '' ) as React.ComponentProps< typeof Dashicon >[ 'icon' ];

		return <Dashicon style={ { padding: '2px' } } icon={ iconKey } aria-hidden="true" />;
	}
