...
import { findDrilldownParent, findDropdownParent, findClosestMenuItem } from './path-matching';
...
export function useSidebarParent() {
	...
	const currentPath = matches[ matches.length - 1 ].pathname.slice( router.options.basepath?.length ?? 0 );
	...
	const [ parentDropdownId, setParentDropdownId ] = useState< string | undefined >(
	...
	return [ parentId, setParentId, parentDropdownId, setParentDropdownId ] as const;
}
