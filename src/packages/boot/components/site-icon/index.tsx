...
function SiteIcon( { className }: { className?: string } ) {
	...
	if ( isRequestingSite && ! siteIconUrl ) {
		...
		icon = siteIconUrl ? (
			<img className="boot-site-icon__image" alt={ __( 'Site Icon' ) } src={ siteIconUrl } />
		) : (
			<Icon className="boot-site-icon__icon" icon={ wordpress } size={ 48 } />
		);
	}

	return <div className={ clsx( className, 'boot-site-icon' ) }>{ icon }</div>;
}
