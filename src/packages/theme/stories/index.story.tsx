 ...
 */
import { useEffect, useState, useRef, useId, createPortal } from '@wordpress/element';
			...
			for ( const rule of sheet.cssRules || [] ) {
				...
				if ( ruleStyle ) {
					for ( const name of ruleStyle ) {
						if ( name.startsWith( '--wp-admin-theme' ) || name.startsWith( '--wp-components-color' ) ) {
							legacyProps[ name ] = ruleStyle.getPropertyValue( name ).trim();
						}
						if ( name.startsWith( '--wpds-color' ) ) {
							if ( name.includes( 'private' ) ) {
								primitiveProps[ name ] = ruleStyle.getPropertyValue( name ).trim();
							} else {
								semanticProps[ name ] = ruleStyle.getPropertyValue( name ).trim();
							}
						}
					}
				}
...
const ColorTokenTable = ( { tokens }: { tokens: Record< string, string > } ) => {
	return (
		<ul
			style={ {
				listStyle: 'none',
	...
	>
		...
		<span
			style={ {
				...
				padding: '0.25rem',
				borderRadius: '0.25rem',
				backgroundColor: 'var(--wpds-color-bg-interactive-brand-weak-disabled)',
				color: 'var(--wpds-color-fg-content-neutral)',
			} }
		>
			Neutral
			...
			<ThemeProvider>
				...
				<div style={ { paddingInlineStart: '1rem' } }>
					...
					>
						...
						<div style={ { paddingInlineStart: '1rem' } }>
							<ThemeProvider>
								<NestingDebug bg="inherit (#1e1e1e)" primary="inherit (root)" />
								<div style={ { paddingInlineStart: '1rem' } }>
									<ThemeProvider color={ { primary: 'hotpink' } }>
										<NestingDebug bg="inherit (#1e1e1e)" primary="hotpink" />
										<div
											style={ {
												paddingInlineStart: '1rem',
											} }
										>
											<ThemeProvider color={ { bg: '#f8f8f8' } }>
												<NestingDebug bg="#f8f8f8" primary="inherit (hotpink)" />
											</ThemeProvider>
										</div>
									</ThemeProvider>
								</div>
...
function IframeWithClonedTokenStyles( { children }: { children: React.ReactNode } ) {
