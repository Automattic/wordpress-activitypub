	...
	it( 'background ramp snapshots', () => {
		...
		expect(
			allBgColors.map( ( bg ) => {
				...
				const seedComputed = serialize( to( parse( ramp.ramp.surface2.color ), sRGB ), {
					format: 'hex',
					inGamut: true,
				} );
		...
		expect(
			allPrimaryColors.map( ( primary ) =>
				options.map( ( o ) => {
					...
					const seedOriginal = serialize( to( parse( primary ), sRGB ), {
						format: 'hex',
						inGamut: true,
					} );
					const seedComputed = serialize( to( parse( ramp.ramp.bgFill1.color ), sRGB ), {
						format: 'hex',
						inGamut: true,
					} );
