...
import { WHITE, BLACK, LIGHTNESS_EPSILON, MAX_BISECTION_ITERATIONS } from './constants';
...
export function findColorMeetingRequirements(
	...
	if ( highestContrast < target ) {
		if ( strict ) {
			throw new Error(
				`Contrast target ${ target.toFixed( 2 ) }:1 unreachable in ${ direction } direction` +
	...
	for ( let i = 0; i < MAX_BISECTION_ITERATIONS; i++ ) {
		...
		const newL =
			( worseL * ( betterContrast - target ) - betterL * ( worseContrast - target ) ) /
