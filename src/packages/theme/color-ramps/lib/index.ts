...
import { clampToGamut, sortByDependency, computeBetterFgColorDirection, adjustContrastTarget } from './utils';

import type { FollowDirection, Ramp, RampDirection, RampConfig, RampResult } from './types';
...
function calculateRamp( {
	...
	const rampResults = {} as Record< keyof Ramp, { color: string; warning: boolean } >;
	...
	for ( const stepName of sortedSteps ) {
		...
		if ( ! referenceColor ) {
			throw new Error( `Reference color for step ${ stepName } not found: ${ contrast.reference }` );
		}
		...
		function computeDirection( color: ColorTypes, followDirection: FollowDirection ): RampDirection {
		...
		const searchResults = findColorMeetingRequirements( referenceColor, seed, adjustedTarget, computedDir, {
			strict: false,
			lightnessConstraint,
			taperChromaOptions,
		} );
...
export function buildRamp(
	...
	const { rampResults, SATISFIED_ALL_CONTRAST_REQUIREMENTS, UNSATISFIED_DIRECTION } = calculateRamp( {
		seed,
		sortedSteps,
		config,
		mainDir,
	...
	if ( ! SATISFIED_ALL_CONTRAST_REQUIREMENTS && rescaleToFitContrastTargets ) {
		...
		for (
			let i = 0;
			i < MAX_BISECTION_ITERATIONS && Math.abs( betterSeedL - worseSeedL ) > LIGHTNESS_EPSILON;
			i++
		) {
			const newSeed = clampToGamut( set( clone( seed ), [ OKLCH, 'l' ], ( worseSeedL + betterSeedL ) / 2 ) );
