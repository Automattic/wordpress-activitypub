 ...
 * @param c
 */
export const clampToGamut = ( c: ColorTypes ) => to( toGamut( c, { space: P3, method: 'css' } ), OKLCH ); // map into Display-P3 using CSS OKLCH method
 ...
 * @param config - The steps configuration object
 */
export function sortByDependency( config: Record< keyof Ramp, RampStepConfig > ): ( keyof Ramp )[] {
...
export function computeBetterFgColorDirection(
	...
	return contrastAgainstBlack > contrastAgainstWhite + ( preferLighter ? WHITE_TEXT_CONTRAST_MARGIN : 0 )
 ...
 */
export function clampAccentScaleReferenceLightness( rawLightness: number, direction: RampDirection ) {
