...
export const RampTable = forwardRef< HTMLDivElement, RampTableProps >( function RampTable( { ramps }, forwardedRef ) {
	return (
		<div style={ { width: '100%', overflowX: 'scroll' } } ref={ forwardedRef }>
			<div
				style={ {
					display: 'grid',
					...
					RAMP_TOKENS_ORDER.map( ( { tokenName } ) => (
						<div
							...
							style={ {
								...
								minWidth: 32,
								fontSize: 14,
								outline: ramp[ tokenName ].warning ? '2px solid red' : '',
								outlineOffset: '-2px',
							} }
						>
							{ tokenName === seed.name ? (
								<div
									style={ {
										...
										fontSize: 8,
										fontWeight: 500,
										color: tokenName === 'surface2' ? ramp.fgSurface4.color : ramp.fgFill.color,
									} }
								>
									SEED
								</div>
							) : null }
							{ [ 'surface3', 'bgFill1', 'bgFillInverted1', 'bgFillDark' ].includes( tokenName ) ? (
								<span
									style={ {
										padding: '2px 6px',
										display: 'flex',
