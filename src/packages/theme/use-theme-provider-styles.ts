...
import { buildBgRamp, buildAccentRamp, DEFAULT_SEED_COLORS, type RampResult } from './color-ramps';
...
function legacyWpAdminThemeOverridesCSS( accent: string ): Entry[] {
	...
	return [
		[ '--wp-admin-theme-color', serialize( to( parsedAccent, sRGB ), { format: 'hex' } ) ],
...
function semanticTokensCSS(
	filterFn: ( entry: [ string, Record< string, string > ] ) => boolean = () => true
): Entry[] {
	return Object.entries( semanticVariables )
		.filter( filterFn )
		.map( ( [ variableName, modesAndValues ] ) => [ variableName, modesAndValues[ '.' ] ] );
}
...
function colorRampCSS( ramp: RampResult, prefix: string ): Entry[] {
	return [ ...Object.entries( ramp.ramp ) ].map( ( [ tokenName, tokenValue ] ) => [
		...
		[
			...
			Array.from( computedColorRamps )
				.map( ( [ rampName, computedColorRamp ] ) => [
					colorRampCSS( computedColorRamp, `--wpds-color-private-${ rampName }-` ),
				] )
				.flat( 2 ),
