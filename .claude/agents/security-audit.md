...
9. **Visibility transitions without a `post_status` change** (2026) — a federated post flipped out of "publicly queryable" by means other than `post_status` (password applied, AP visibility meta → `local`/`private`, post type losing `activitypub` support) kept exposing its content, because the scheduler emitted `Update` instead of `Delete`, content-derivation helpers read raw fields with no gate, and the content-negotiation surface still rendered it. Outbox snapshots freeze any such leak until purged. **Pattern:** every transition out of publicly-queryable needs (i) the scheduler to emit `Delete`, (ii) every content-derivation helper to refuse raw reads, and (iii) the content-negotiation surface to refuse to render. See Audit Scope §1.
...
15. **Remote-supplied URL-shaped values reaching a link or navigation sink** (2026) — patterns to enforce, stated generally so the audit finds the whole class. A value that arrives inside a remote document (a WebFinger link `template`, an actor field, any remote JSON member that later becomes an `href`, a `window.open()` target, or a redirect) is attacker-controlled to the same degree as an inbox activity, and *escaping* it is not the same as *validating* it. (a) Gate on an explicit scheme allow list rather than escaping: `esc_url()` is the wrong tool for URI templates because it strips the `{uri}` / `{object}` placeholders those formats require, and the default `wp_allowed_protocols()` is much wider *and* filterable, so a third-party plugin can otherwise widen a security gate it does not know exists. (b) A scheme check alone is not sufficient: protocol-relative (`//host`), schemeless-path, empty-authority (`https:///path`), and empty or whitespace-only values all give `wp_kses_bad_protocol()` nothing to strip and sail through, so pair it with a host requirement (`get_url_authority()`), and compare case-folded because the kses pass lowercases the scheme it returns. Server-side the gate is `get_url_authority()` + `wp_kses_bad_protocol( $value, array( 'http', 'https' ) )` + a case-folded compare; client-side it is `isSafeUrl()` in `src/shared/safe-url.js`. Do not substitute `filter_var( …, FILTER_VALIDATE_URL )` or `wp_http_validate_url()` here: the former only accepts ASCII hosts (rejecting IDN hosts the browser's `new URL()` accepts, so the two sides of the wire diverge), and the latter enforces fetch-oriented port and private-IP policies that do not apply to a URL the visitor's browser opens. (c) Enforce on **both** sides of the wire: the server-side gate where the value is stored or returned, *and* the client-side gate where it reaches the sink; the two helpers are mirrors, keep them in step. (d) Anything already persisted from before such a gate existed (`localStorage`, options, post meta) must be re-validated **on read** — adding the gate on write does not clean records that are already stored. **Detection:** trace every remote-document field that ends up in `window.open(`, an `href`, `wp_redirect()`, or a `data-wp-bind--href`, and confirm a scheme allow list sits somewhere on that path. Skipping a bad value rather than erroring keeps sibling links and fallbacks working. See Audit Scope §7.
16. **OAuth scope is a second check next to identity, on every path that reaches the data** (2026) — patterns to enforce, stated generally so the audit finds the whole class. Any valid bearer token on the plugin's namespace establishes the WordPress user, whatever it was consented to, so `is_user_logged_in()`, `get_current_user_id()`, `current_user_can()` and `verify_owner()` answer *who* the caller is, never *what* the token may do. (a) Every owner-only read must also require `read`, every mutation `write`, whatever transport delivers the data: a stream or a POST that only reads is still a read. Use the shared gates (`verify_authentication()`, `owner_may_read()`, `OAuth_Server::permits_scope()`) instead of a bare login or ownership check. (b) The same data usually has several read paths: paged collection, event stream, single-item permalink, embedded or seek responses, and non-REST callers that authenticate a bearer themselves. Each one must apply the same scope rule as the paged read; a fix to one path is not a fix to the class. (c) Shared code that branches per collection or per activity type must apply the rule in every branch, with a test for every branch, including a scope-limited token as the negative case. See Audit Scope §5.
...
- Map every code path that trusts the current user on the plugin's namespace (grep for `is_user_logged_in`, `get_current_user_id`, `current_user_can`, `verify_owner`, and direct bearer authentication outside REST dispatch) and confirm each one also checks the scope that matches the action. A path that only checks identity is a finding (see finding #16).
- For each collection, list all of its read paths and confirm they agree on the required scope. Check each branch of shared stream or collection code separately.
- Confirm tests cover a token that lacks the needed scope for every path and branch, not only the one that was reported.
...
**Baseline: `wp_safe_remote_get/post()` blocks loopback and RFC1918, with one known gap.** It routes through core's `wp_http_validate_url()`, which permits **link-local `169.254.0.0/16`** (the cloud-metadata IP `169.254.169.254`) and **CGNAT `100.64.0.0/10`**. This gap is **known and tracked upstream — a WordPress core patch to `wp_http_validate_url()` is pending** (see finding #12). Do not re-report it as a new finding; instead check whether the core patch has landed. The residual exposure is blind (reachability/timing oracle, no body disclosure).

**The plugin's own stricter guard is `resolve_public_host()`** (`includes/functions-request.php`): `FILTER_FLAG_NO_PRIV_RANGE | FILTER_FLAG_NO_RES_RANGE` rejects link-local, plus `is_unsafe_ipv6_literal()`. CGNAT (`100.64.0.0/10`) is **deliberately not blocked** — PHP's filter flags treat it as public, and so do we; see finding #12 for the trade-off. It is applied on bespoke connection paths that bypass the WP HTTP API's protections or need pre-connect validation (`oauth/class-client.php`, `rest/trait-event-stream.php`) — **not** inside `Http::get()`/`Http::post()`, which currently rely on core's validation. The `activitypub_allow_non_public_host` filter opts intranet deployments out of the strict guard; a fix that adds `resolve_public_host()` to a new path must keep that filter working or it breaks private-network federation.
...
- Any remote-supplied value that becomes a URL in a sink (`href`, `window.open()`, `wp_redirect()`, `data-wp-bind--href`) must pass a scheme allow list, not just escaping — the recipe lives in history #15.
...
### 10. Remote Object Cache Integrity (fetch → write → read binding)

Files: `includes/collection/class-remote-actors.php`, `includes/collection/class-remote-posts.php`, `includes/collection/class-interactions.php`, `includes/class-http.php`, `includes/functions-request.php`, `includes/handler/`, and any store keyed by a remote identifier.

**The general rule: whenever the plugin persists remote-supplied data into a cache/store keyed by a remote-controlled identifier (an `id`, `url`, `guid`, `acct`/handle, `keyId`, `serverId`, …), the write MUST be bound to a trusted origin.** If it is not, one remote host can write a record under *another* host's identifier — poisoning it. This is a class, not a single bug: it applies to actors, keys, posts, comments/interactions, followers, tombstones, and any future remote store. History #11 is one instance; audit for the whole class every time.

**Enumerate every remote-id-keyed write and classify its trust source.** Grep for the write chokepoints (`upsert`, `create`/`update`, `wp_insert_*`/`wp_update_*`/`add_*_meta`/`update_option`) and, for each, find the identifier the row is keyed by and where that identifier comes from. There are exactly two legitimate trust models — every write must use one:

1. **Dereferenced-by-URL objects** (actors, public keys, collections, or any object fetched over HTTP by its URL). Trust the document ONLY when it is served under its own `id`. If the fetched document declares a *different* `id`, re-fetch that `id` from **its own host** and accept only the self-confirmed copy (`id === served-from-url`, compared against the **post-redirect** URL); a **single** re-fetch, no chasing (loop guard). This mirrors Mastodon's `JsonLdHelper#fetch_resource`, and the plugin builds it into `Http::get_remote_object()` itself (via the generic `id_matches_url()` helper), so every caller that fetches by URL is self-confirmed by default — there is no separate per-caller dereference step to look for. When auditing, verify that new cache writes obtain their document through `Http::get_remote_object()` (not a raw `Http::get()` that skips the binding).
2. **Embedded objects inside a signed activity** (e.g. a Create's post, a reaction's activity id). The outer HTTP signature vouches for the *content* but NOT for cross-host `id` claims, so bind the stored identifier's host to the signing actor's host: `host(object.id) === host(activity.actor)`. Do not re-fetch (that defeats embedding and gains nothing).

**Trace fetch → write → read as ONE chain.** A poisoned write is only dangerous because a later read trusts it. For each store, find the read side (`get_public_key`, `get_by_uri`/`get_by_guid`, dedup lookups like `object_id_to_comment`/`url_to_commentid`) and confirm the read cannot be steered to a cross-actor record — and that the read does not silently re-serve a poisoned entry without re-validating.

**Red flags to grep for:**
- A **two-key mismatch**: the lookup that decides "does this exist?" uses a *different* key than the write (e.g. `fetch_by_uri` checks `get_by_uri($url)` but `upsert()` keys on `$object['id']`). The gap between the two keys is where cross-entry overwrite lives.
- Any `upsert`/`create`/`insert` keyed by `$object['id']` / `$activity['object']['id']` / a `guid` taken straight from remote input, with no origin binding above it.
- A cache **read** (key lookup) that returns a stored record to a security decision (signature verification, ownership, addressing) without confirming the record's origin.
