...
- **Disable redirects for that fetch (`redirection => 0`), or validate the *final* response URL's host.** `Http::get()` follows redirects by default, so without this an open redirect on the (trusted) named host bounces to attacker-controlled JSON while the host-equality check still sees the original host. (Precedent: `includes/oauth/class-client.php` fetches CIMDs with `redirection => 0` "to prevent client impersonation".)
