...
15. **Remote-supplied URL-shaped values reaching a link or navigation sink** (2026) — patterns to enforce, stated generally so the audit finds the whole class. A value that arrives inside a remote document (a WebFinger link `template`, an actor field, any remote JSON member that later becomes an `href`, a `window.open()` target, or a redirect) is attacker-controlled to the same degree as an inbox activity, and *escaping* it is not the same as *validating* it. (a) Gate on an explicit scheme allow list rather than escaping: `esc_url()` is the wrong tool for URI templates because it strips the `{uri}` / `{object}` placeholders those formats require, so the check is `wp_kses_bad_protocol( $value, array( 'http', 'https' ) )`. (b) Pass that list explicitly — the default `wp_allowed_protocols()` is much wider *and* is filterable, so a third-party plugin can otherwise widen a security gate it does not know exists. (c) A scheme check alone is not sufficient: `wp_kses_bad_protocol()` inspects only the scheme, so pair it with an authority check (`get_url_authority()`), or protocol-relative (`//host`), schemeless-path, and empty-authority (`https:///path`) values all pass. (d) Compare case-insensitively, the way `wp_http_validate_url()` does — schemes are case-insensitive, so an uppercase one otherwise looks like a rewrite. (e) Enforce on **both** sides of the wire: the server-side gate where the value is stored or returned, *and* the client-side gate where it reaches the sink. When the two allow lists are mirrored (a PHP helper and a JS helper), treat them as one unit and keep them in step. (f) Anything already persisted from before such a gate existed (`localStorage`, options, post meta) must be re-validated **on read** — adding the gate on write does not clean records that are already stored. **Detection:** trace every remote-document field that ends up in `window.open(`, an `href`, `wp_redirect()`, or a `data-wp-bind--href`, and confirm a scheme allow list sits somewhere on that path. Skipping a bad value rather than erroring keeps sibling links and fallbacks working. See Audit Scope §7.
...
- Any remote-supplied value that becomes a URL in a sink (`href`, `window.open()`, `wp_redirect()`, `data-wp-bind--href`) must pass a scheme allow list, not just escaping — `wp_kses_bad_protocol( $value, array( 'http', 'https' ) )` with the list passed **explicitly** (the default `wp_allowed_protocols()` is wider and filterable), plus an authority check, compared case-insensitively. Check both the server-side gate and the client-side one, and confirm values persisted before the gate existed are re-validated on read (see history #15).
